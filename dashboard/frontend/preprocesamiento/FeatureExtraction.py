...
class FeatureExtraction:
    ...
    def extraction(self, extractor):
        ...
        if X_train.shape[0] > 15000 and y_train.shape[0] > 15000:
            print(
                f"Bag of Words: Muestra mayor a 20,000 registros: {X_train.shape[0]} Reduciendolo 15000...")
        ...
        X_resampled_path = Path(
            self.ruta_actual / ("DB/Resampling_SMOTE/X_resampled_" + extractor+"_"+self.idioma))
        y_resampled_path = Path(
            self.ruta_actual / ("DB/Resampling_SMOTE/y_resampled_" + extractor+"_"+self.idioma))
        if(X_resampled_path.exists() and y_resampled_path.exists()):
            X_resampled = self.loadPickle(
                "DB/Resampling_SMOTE/X_resampled_" + extractor+"_"+self.idioma)
            y_resampled = self.loadPickle(
                "DB/Resampling_SMOTE/y_resampled_" + extractor+"_"+self.idioma)
        else:
        ...
        NB_clf = self.MultiNaiveBayes(
            X_resampled, X_features_test, y_resampled, y_test)
        self.crearPickle(NB_clf, 'Classifiers/MultinomialNB_' + self.idioma)

        print("Modelado: LogisticRegression...")
        LR_clf = self.LogisticRegression(
            X_resampled, X_features_test, y_resampled, y_test)
        self.crearPickle(
    ...
    def bagOfWords(self, X_train, X_test):
        ...
        self.crearPickle(count_vectorizer,
                         'Vectorizers/CountVectorizer_' + self.idioma)
    ...
    def overSamplingSMOTE(self, X_train, y_train, vectorizer, extractor):
        ...
        X_resampled, y_resampled = method.fit_sample(
            X_df.to_numpy(), y_train.to_numpy())
        print(pd.value_counts(pd.Series(y_resampled)))
        # Guardar en Pickles
        self.crearPickle(
            X_resampled, "DB/Resampling_SMOTE/X_resampled_" + extractor + "_" + self.idioma)
        self.crearPickle(
            y_resampled, "DB/Resampling_SMOTE/y_resampled_" + extractor + "_" + self.idioma)
